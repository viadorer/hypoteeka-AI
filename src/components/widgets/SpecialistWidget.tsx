...
// Default fallback — používá se, pokud Hugo nezavolal show_specialists s brokerIds
// a backend nedohledal žádné brokery (Supabase down). Aktuálně = David Choc.
...
export function SpecialistWidget({ specialists }: { specialists?: Specialist[] }) {
  const list = specialists && specialists.length > 0 ? specialists : DEFAULT_SPECIALISTS;
  // U jednoho specialisty rozbalit detail rovnou (žádné klikání).
  const initialExpanded = list.length === 1 ? 0 : null;
  ...
  return (
    <div className="bg-[#ffffff] rounded-2xl p-4 md:p-6 shadow-[0_20px_50px_rgba(0,26,65,0.08)] border border-[#e4bdc2]/10 animate-in slide-in-from-bottom-4 duration-500 overflow-hidden w-full min-w-0">
      ...
      <p className="text-[11px] font-semibold uppercase tracking-wider text-[#001a41]/40 mb-0.5">
        ...
        {list.length === 1 ? 'Váš specialista' : 'Dostupní specialisté'}
      </p>
